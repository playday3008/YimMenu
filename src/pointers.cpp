...
namespace big
{
	constexpr auto pointers::get_gta_batch()
	{
        ...
        constexpr auto data = std::to_array<memory::signature>({
            // Screen Resolution
            {
                "SR",
		...
		constexpr memory::batch<data.size()> batch(data);

		constexpr auto batch_and_hash = memory::batch_and_hash<data.size()>{
            "GTA5",
            batch,
            memory::signature_hasher::add(data)
        };
	    ...
	    m_sc_pointers_cache(g_file_manager.get_project_file("./cache/sc_pointers.bin"))
	{
		...
		static_assert(gta_batch_and_hash.m_hash != 0, "Hash must be non-zero");
		write_to_cache_or_read_from_cache<
		    gta_batch_and_hash.size(),
		    gta_pointers_layout_info::offset_of_cache_begin_field,
		    gta_pointers_layout_info::offset_of_cache_end_field
		>(gta_batch_and_hash, m_gta_pointers_cache, mem_region);
		...
		if (sc_module.wait_for_module())
		{
			...
			static_assert(sc_batch_and_hash.m_hash != 0, "Hash must be non-zero");
			write_to_cache_or_read_from_cache<
			    sc_batch_and_hash.size(),
			    sc_pointers_layout_info::offset_of_cache_begin_field,
			    sc_pointers_layout_info::offset_of_cache_end_field
			>(sc_batch_and_hash, m_sc_pointers_cache, sc_module);
		}
		else
