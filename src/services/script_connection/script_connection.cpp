...
namespace big
{
	...
	void script_connection::start_script_on_remote()
	{
		...
		else
		{
			...
			int64_t args[arg_count] = {(int64_t)eRemoteEvent::StartScriptBegin, (int64_t)self::id, 1i64 << m_target->get()->id()};

			args[2] = scripts::launcher_index_from_hash(m_script_hash);
			::strcpy_s((char*)&args[2 + 3], (arg_count - 2 - 3 - (arg_count - 16)) * sizeof(*args), "0");
			args[2 + 16] = -1;
			args[2 + 17] = 1337;
			...
			for (int i = 0; i < 2; i++)
			{
				...
				int64_t args_2[arg_count_2] = {(int64_t)eRemoteEvent::StartScriptProceed, (int64_t)self::id, 1i64 << m_target->get()->id()};
				args_2[2 + 17]              = 1337;
