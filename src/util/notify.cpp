	...
	void crash_blocked(CNetGamePlayer* player, const char* crash)
	{
		if (player)
		{
			...
			if (g.reactions.crash.notify)
				g_notification_service.push_error("PROTECTIONS"_T.data(), std::format("Blocked {} crash from {}", crash, player->get_name()));
		...
		{
			if (g.reactions.crash.notify)
				g_notification_service.push_error("PROTECTIONS"_T.data(), std::format("Blocked {} crash from unknown player", crash));
		}
	}
